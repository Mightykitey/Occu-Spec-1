...
function new_staff($conn, $post)
{
    try {
        ...
        $sql = "INSERT INTO staff (fname, lname, email,pwd) VALUES (?,?,?,?)"; // Using a prepared statement for security
...
function S_audititor($conn, $stfid, $job, $long){
    $sql = "INSERT INTO saudit (date, desc, staff_id) VALUES (?,?,?)";
...
function get_staff($conn){
    $sql = "SELECT staffid,fname, lname, email, pwd FROM staff where job != ? ORDER BY job DESC";
